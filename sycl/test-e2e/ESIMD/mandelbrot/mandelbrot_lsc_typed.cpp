//==------ mandelbrot_lsc_typed.cpp - DPC++ ESIMD on-device test ----------==//
//
...
// RUN: %{run} %t.out
//
// Example application demonstrating the Xe2+ typed-surface (image) RGBA scatter
// ESIMD API lsc_scatter_rgba_typed (the ESIMD equivalent of the CM
// cm_store4_typed operation). Each work-item computes the Mandelbrot escape
...
int main() {
  ...
  try {
    ...
    q.submit([&](handler &cgh) {
       ...
       cgh.parallel_for<class MandelLscTyped>(
           GlobalRange, [=](item<2> it) SYCL_ESIMD_KERNEL {
             ...
             iexp::lsc_scatter_rgba_typed<uint32_t, N>(Acc, U, V, R, LOD, Px);
           });
     }).wait();
  ...
  std::cout << "Mismatching pixels: " << NumDiff << " (" << (DiffRatio * 100.0)
            << "%)\n";
